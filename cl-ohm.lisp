...
(defgeneric counter-value (model slot-name)
  ...
  (:method ((model ohm-model) slot-name)
    (let ((slot (find-slot (class-of model) slot-name)))
      (when (counterp slot)
        (with-connection ()
          (let ((counter (red:hget (object-key model '_counters) slot-name)))
            (when counter
              (parse-integer counter :junk-allowed t))))))))

(defgeneric incr (model slot-name &optional increment)
  (:documentation "Increments the counter named SLOT-NAME by VALUE.
This implicitly saves the model to the data store.")
  (:method :before ((model ohm-model) slot-name &optional increment)
           (declare (ignore slot-name increment))
           (ensure-managed-object model))
  (:method ((model ohm-model) slot-name &optional (increment 1))
    (let ((slot (find-slot (class-of model) slot-name)))
      (when (counterp slot)
        (with-connection ()
          (red:hincrby (object-key model '_counters) slot-name increment))))))

(defgeneric decr (model slot-name &optional decrement)
  (:documentation "Decrements the counter named SLOT-NAME by VALUE.
This implicitly saves the model to the data store.")
  (:method :before ((model ohm-model) slot-name &optional decrement)
           (declare (ignore slot-name decrement))
           (ensure-managed-object model))
  (:method ((model ohm-model) slot-name &optional (decrement 1))
    (let ((slot (find-slot (class-of model) slot-name)))
      (when (counterp slot)
        (with-connection ()
          (incr model slot-name (- decrement)))))))
